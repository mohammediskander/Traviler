...
/**
 Router protocol for Yelp API,
 This Router is mainly used to get data from `https://api.yelp.com`.
 
 API Key: The authenticaiton token thet've been generated by Yelp API to allow communication between client (Taviler Application) and server (Yelp API). This key generated from a free account, so their will be some limitation in data that'll be provided by API provider.
 
 Endpoints:
    * Each endpoint is written as : `case [HTTP METHOD][METHOD NAME]`, but in `camleCase`. (see `case getBusinessSearch`)
        ...
        - After each endpoint decleration, the remote URL for the API docs of that is endpoint is provided. (see `case getBusinessSearch`)
 */
